...
| `GET /paths/flow` | 10 | 1 | 27,452 | 1,778 | 5 ms | private, max-age=300 |
...
- `/network/saved` is paginated at the 100 cap and both readers request one page; a member who saved more than 100 people needs an ids endpoint or a pager.
...
- Gates at the end of the pass: `poe lint` and `poe format-check` clean; `poe test-fast` 572 passed; integration 249 passed and the one pre-existing auth failure above; `npm run typecheck`, `npm run lint`, `npm run build` clean; OpenAPI regenerated.
