...
from backend.housing.domain import HousingListing, HousingStatus
from infrastructure.repository import utc_now


def is_owner(row: HousingListing, actor: Actor | None) -> bool:
    if actor is None or actor.member_id is None:
        return False
    return row.member_id == actor.member_id


def is_on_the_board(row: HousingListing, actor: Actor | None) -> bool:
...
def for_viewer(row: HousingListing, actor: Actor | None) -> HousingListing:
    ...
    being told" is a different answer from "nobody has looked".
    """
    if is_owner(row, actor) or (actor is not None and actor.is_admin):
        return row
